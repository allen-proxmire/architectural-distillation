...
**Quantitatively, the closure resists.** Across all four rounds, *no* clean closed continuum equation was verified — not as a ρ-PDE (R1), not as a `(ρ,φ,J)` transport system (R2–3), not even with ensemble averaging + matched space-time coarse-graining (R4: continuity R²≈0.15, degrading with scale). The kinetic *equilibrium* is clean (isotropic, τ≈1.5), but the *hydrodynamic closure* is not. The honest standing: the certified substrate's continuum is **qualitatively kinetic and quantitatively unclosed** under the methods tried — a real limit on its generativity, and the program's clearest tested negative.
...
## 8. Round-4 outcome and the Round-5 fork

**Round 4 (done).** Kinetic equilibrium clean (isotropic `f_d`, τ≈1.5); continuity still does not close (R²≈0.15, degrading); **substrate ≠ UDM** (mobility concentration-independent, β≈0). The over-damped→UDM connection does not hold as run.

**Round 5 (the one live fork).** Re-run with the **saturation/extinction threshold** active. Fronts die where Σ falls below threshold — i.e. in dense, high-ρ regions — which is the *blocking* mechanism UDM requires. Test whether effective transport into dense regions then degenerates with concentration as `(1−c/c_max)^β`. Outcome decides between **"UDM is unrelated to the substrate"** and **"UDM is the substrate's saturation regime."** Principled (saturation is the natural blocking mechanism), not a fit.

**Beyond Round 5 (open, not scoped here).** Whether *any* coarse-graining yields a clean closed PDE for this substrate (it may not, given the unbounded-ρ non-steady regime); and the velocity-resolved Chapman-Enskog derivation of the effective transport coefficient.
...
*Result: the certified ED substrate generates kinetic lattice-gas continuum dynamics (ballistic worldlines + disorder scattering, isotropic fast-relaxing equilibrium, slaved ρ) — not diffusion, not a closed scalar PDE. Quantitatively the continuum does **not** close under the methods tried (R1–4), and the substrate does **not** generate the UDM mobility law (β≈0, not β≈2: avoidance, not blocking) — the Q-C/UDM diffusion is a different ED model. Qualitative kinetic verdict solid; quantitative closure unachieved; the saturation/extinction mechanism is the one live UDM fork (Round 5). No new rules; certified simulator only; the program's first **tested** (not merely located) substrate result — and its clearest tested negative.*
